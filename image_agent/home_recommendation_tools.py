...
def ProductMatcher(room_description: str, user_context: dict) -> list:
    ...
    if "bright" in room_description.lower():
        query += " AND brightness_nits > 1000"
    elif "dark" in room_description.lower():
        query += " AND brightness_nits < 1000"
...
def RecommendationExplainer(products: list, room_description: str, user_context: dict) -> str:
    ...
    response = model.generate_content(
        ...
        Please provide a human-readable explanation for why these products are a good fit for the user, considering the user's context and the room description.
        """
    )
    return response.text
